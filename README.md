...
- Normalizes Signal desktop HTML exports generated by `signalbackup-tools --exportdesktophtml`
...
- `EXPORT_CRON` (default: `0 2 * * 0`) - extra full refresh + rebuild schedule, typically used for a different Discord cadence.
...
For desktop/docker Signal data, the scheduler runs `signalbackup-tools --exportdesktophtml` into a temporary directory and then atomically replaces `data/signal_decrypted/`.
...
This produces local HTML exports under `data/discord/html/` and media under `data/discord/media/`.

The scheduler replaces `data/discord/html/` on each run but keeps `data/discord/media/` so `--reuse-media` can dramatically reduce bandwidth.
...
If both `DISCORD_EXPORT_COMMAND` and `DISCORD_BOT_TOKEN` are empty, the Discord export step logs a skip and the rebuild continues with the existing on-disk data.

If you provide a custom `DISCORD_EXPORT_COMMAND`, it should write HTML output into `$DISCORD_EXPORT_DIR`. You can also write or reuse media in `$DISCORD_MEDIA_DIR`.
