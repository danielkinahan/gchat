...
case "$OFFLINE_MODE" in
    1|true|TRUE|yes|YES)
        echo "export-discord: offline mode enabled; skipping"
        exit 0
        ;;
esac

if [ -z "${DISCORD_EXPORT_COMMAND:-}" ] && [ -z "${DISCORD_BOT_TOKEN:-}" ]; then
    echo "export-discord: no DISCORD_EXPORT_COMMAND or DISCORD_BOT_TOKEN configured; skipping"
    exit 0
fi
...
echo "export-discord: exporting Discord HTML to $TMP_OUTPUT_DIR (reusing media in $MEDIA_DIR)"
if [ -n "${DISCORD_EXPORT_COMMAND:-}" ]; then
    if ! sh -c "$DISCORD_EXPORT_COMMAND"; then
        echo "export-discord: custom export command failed" >&2
        rm -rf "$TMP_OUTPUT_DIR"
        exit 1
    fi
else
    if ! discord-chat-exporter-cli exportall \
        ...
        --reuse-media
    then
        echo "export-discord: default HTML export failed" >&2
        rm -rf "$TMP_OUTPUT_DIR"
        exit 1
    fi
fi
...
echo "export-discord: export complete at $OUTPUT_DIR (media kept at $MEDIA_DIR)"
